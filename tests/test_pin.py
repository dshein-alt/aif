...
def make_thread(cli, body="the topic of this thread", **extra):
    return cli.post("/api/threads", json={"subject": "the subject", "b": body, **extra}, headers={"x-agent": "a1"}).json()
...
def test_the_first_message_is_returned_as_the_description(cli):
    ...
    tid = made["t"]
    cli.post(f"/api/threads/{tid}/msgs", json={"b": "first reply"}, headers={"x-agent": "a2"})
...
def test_the_description_survives_every_way_of_paging(cli):
    ...
    tid = made["t"]
    for i in range(5):
        cli.post(f"/api/threads/{tid}/msgs", json={"b": f"reply {i}"}, headers={"x-agent": "a2"})
    variants = {
...
def test_deleting_the_opener_passes_the_description_to_the_next_message(cli):
    ...
    tid = made["t"]
    second = cli.post(f"/api/threads/{tid}/msgs", json={"b": "second description"}, headers={"x-agent": "a2"}).json()
    gone = cli.post("/api/op", json={"do": "rm", "what": "message", "id": made["i"]}, headers={"x-agent": "a1"})
    assert gone.json()["ok"] == 1
    page = cli.get(f"/api/threads/{tid}").json()
    assert page["pin"]["i"] == second["i"] and page["pin"]["b"] == "second description"
    cli.post("/api/op", json={"do": "rm", "what": "message", "id": second["i"]}, headers={"x-agent": "a2"})
...
def test_the_ui_shows_the_description_on_later_pages(cli, tmp_path):
    ...
    for i in range(3):
        ui.post(f"/api/threads/{made['t']}/msgs", json={"b": f"reply {i}"}, headers={"x-agent": "a2"})
    page = ui.get(f"/ui/thread/{made['t']}?token={TOKEN}&since={made['i']}").text
